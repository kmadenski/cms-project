...
final class ResolveMediaObjectContentUrlSubscriber implements EventSubscriberInterface
{
    ...
    public function onPreSerialize(GetResponseForControllerResultEvent $event): void
    {
        ...
        if (!($attributes = RequestAttributesExtractor::extractAttributes($request)) || !\is_a($attributes['resource_class'], UserAvatar::class, true)) {
            return;
        }
        ...
        foreach ($mediaObjects as $mediaObject) {
            if (!$mediaObject instanceof UserAvatar) {
                continue;
            }
